...
(meditations
  ...
  ;; but `get` is explicit and better :)
  ...
  (= :key-not-found-my-dudes (get {:a 1 :b 2} :c :key-not-found-my-dudes))
  ...
  (= {1 "January" 2 "February"} (assoc {1 "January"} 2 "February")) ;; well it is also works for vectors (assoc [1 2] 1 3) => [1 3]

  "You can also create a new version with an entry removed"
  (= {1 "January"} (dissoc {1 "January" 2 "February"} 2)) ;; but dissoc can not be applied to vectors: perfectionists on fire
